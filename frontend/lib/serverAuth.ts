...
import { pickHighestPriorityRole, toAppRole } from '@/lib/authRoles';
import { getSupabaseAdminClient } from '@/lib/supabaseAdmin';

type AppRole = 'super-admin' | 'admin' | 'instructor' | 'account' | string | null;
...
export async function getAuthenticatedAuthUserId(
  ...
  if (error) {
    throw new AuthContextError(`Failed to resolve authenticated user: ${error.message}`, {
      status: 401,
...
export async function getCurrentPersonFromRequest(
  ...
  if (personError) {
    throw new AuthContextError(`Failed to load person for authenticated user: ${personError.message}`, {
      status: 500,
  ...
  if (!person?.person_id) {
    throw new AuthContextError('No person record is linked to this authenticated user.', {
      status: 404,
  ...
  const personOrganizationIds = memberships.map((item) => item.person_organization_id);

  const { data: personOrgRoles, error: personOrgRolesError } = personOrganizationIds.length
    ? await supabaseAdmin
  ...
  if (personOrgRolesError) {
    throw new AuthContextError(`Failed to load organization roles: ${personOrgRolesError.message}`, {
      status: 500,
      code: 'person_org_roles_failed',
    });
  }

  const allRoleIds = Array.from(
    new Set([
      ...((personOrgRoles ?? []) as PersonOrgRoleRow[]).map((item) => item.role_id),
      ...((personGlobalRoles ?? []) as PersonGlobalRoleRow[]).map((item) => item.role_id),
    ])
  );
  ...
  if (roleRowsError) {
    throw new AuthContextError(`Failed to load role names: ${roleRowsError.message}`, {
      status: 500,
  ...
  ((personOrgRoles ?? []) as PersonOrgRoleRow[]).forEach((item) => {
    ...
    const currentRoleNames = rolesByPersonOrganizationId.get(item.person_organization_id) ?? [];
    currentRoleNames.push(roleName);
    rolesByPersonOrganizationId.set(item.person_organization_id, currentRoleNames);
  });
  ...
  const organizationRoleNames = Array.from(rolesByPersonOrganizationId.values()).flat();
  const roleNames = Array.from(new Set([...globalRoles, ...organizationRoleNames]));
  ...
  return {
    ...
    memberships: memberships.map((membership) => ({
      ...
      roles: rolesByPersonOrganizationId.get(membership.person_organization_id) ?? [],
    })),
    roleNames,
    dbRole,
...
export async function requireCurrentPersonFromRequest(
  ...
  if (!context) {
    throw new AuthContextError('No authenticated session was provided with this request.', {
      status: 401,
