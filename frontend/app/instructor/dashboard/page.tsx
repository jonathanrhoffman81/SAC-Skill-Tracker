...
export default function InstructorDashboard() {
  ...
  async function loadDashboardData(
    ...
    try {
      ...
      const resolvedOrganizationName = payload.organizationName || 'SAC Skill Tracker';
      ...
      if (!activeSearch && activePage === 1) {
        ...
        if (!cacheRef.current.has(oppositeCacheKey)) {
          const oppositeEndpoint = oppositeTab === 'all' ? '/api/instructor/all-swimmers' : '/api/instructor/dashboard';
          ...
          fetch(`${oppositeEndpoint}?${oppositeParams.toString()}`, { headers })
            .then(async (prefetchResponse) => {
              ...
              const prefetchPayload = (await prefetchResponse.json()) as DashboardPayload;
              ...
              cacheRef.current.set(oppositeCacheKey, {
                ...
                organizationName: prefetchPayload.organizationName || 'SAC Skill Tracker',
      ...
      const message = fetchError instanceof Error ? fetchError.message : 'Unexpected error loading dashboard.';
      setError(message);
      setSwimmers([]);
      setPagination({
        page: 1,
                    ...
                    <div className="border-t border-gray-100 p-5 sm:p-6">
                      <EvaluationForm
                        ...
                        onSubmissionComplete={() =>
                          loadDashboardData(swimmerTab, currentPage, debouncedSearchQuery, { forceRefresh: true })
                        }
                      />
                    </div>
                  )}
