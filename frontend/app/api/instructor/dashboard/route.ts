...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get enrollments in instructor's groups to find their specific swimmers
    const { data: instructorEnrollments, error: enrollmentsError } =
      await supabaseAdmin
        .from("enrollment")
        .select("member_id")
        .in("group_id", Array.from(instructorGroupIds));

    if (enrollmentsError) {
      console.error("Instructor enrollments error:", enrollmentsError);
      return NextResponse.json(
        {
          error: `Failed to load instructor enrollments: ${enrollmentsError.message}`,
        },
        { status: 500 },
      );
    }

    // Extract unique member IDs from instructor's groups
    const instructorMemberIds = Array.from(
      new Set((instructorEnrollments ?? []).map((e) => e.member_id)),
    );

    // If instructor has no students, return empty list
    if (instructorMemberIds.length === 0) {
      return NextResponse.json({
        userName,
        organizationName: "SAC Skill Tracker",
        organizationLogoUrl: getOrganizationLogoUrl(organizationId),
    ...
    let memberCountQuery = supabaseAdmin
      ...
      .in("member_id", instructorMemberIds);
    ...
    const swimmers: DashboardSwimmerPayload[] = members.map((member) => {
      ...
      const needsEvaluation = memberEnrollments.some(enrol => {
        ...
        // Check if there's already a recent evaluation for this class
        ...
        if (recentEval) {
          ...
          console.log(`  [EVAL CHECK] Swimmer evaluated ${evalDiffDays} days ago, class ends in ${diffDays} days`);
          
          // If eval is recent (within 14 days), don't need new one regardless of class date
          if (evalDiffDays <= 14) {
            console.log(`    → Recent evaluation found (${evalDiffDays} days old), returning false`);
            return false;
          }
        }

        // Check if class is ending within next 3 days
        if (diffDays <= 3 && diffDays >= 0) {
          console.log(`  → Class ending soon (in ${diffDays} days), returning true`);
          return true;
        }

        // Check if class ended within last 14 days (with no recent eval)
        if (diffDays < 0 && diffDays >= -14) {
          console.log(`  → Class ended recently (${Math.abs(diffDays)} days ago), no recent eval found, returning true`);
          return true;
        }
