...
async function buildParentSwimmerProfile(
  email: string,
  memberId: string,
  ...
  if (!accessCheck.ok) {
    const accessError =
      accessCheck.error ?? "You do not have access to this swimmer.";
    const prefixedError = accessError.startsWith("Failed")
      ? accessError
  ...
  const [
    ...
    supabaseAdmin
      .from("evaluation")
      .select(
        "evaluation_id, feedback, evaluation_date, instructor_person_id, skill_id",
      )
      .eq("member_id", memberId)
  ...
  if (orgSkillsError) {
    throw new Error(
      `Failed to load organization skills: ${orgSkillsError.message}`,
    );
  }
  ...
  if (authorIds.length > 0) {
    ...
    if (authorRowsError) {
      throw new Error(
        `Failed to load note authors: ${authorRowsError.message}`,
      );
    }
